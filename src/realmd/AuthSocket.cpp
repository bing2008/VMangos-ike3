...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    else
    {
        ...
        result = LoginDatabase.PQuery("SELECT sessionkey,id,locked,last_ip,v,s,security,email_verif,geolock_pin,email,UNIX_TIMESTAMP(joindate) FROM account WHERE username = '%s'",_safelogin.c_str ());
        if (result)
        {
